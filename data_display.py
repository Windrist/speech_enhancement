...
def make_3plots_spec_voice_noise(stftvoicenoise_mag_db, stftnoise_mag_db, stftvoice_mag_db, sample_rate,
    ...
    plt.colorbar()
    plt.subplot(3, 1, 2)
    plt.title('Spectrogram Predicted Noise')
...
def make_2plots_spec_clean_out(stftout_mag_db, stftvoice_mag_db, sample_rate, hop_length_fft):
    """This function plots the spectrograms of clean voice and predict voice as a single plot """
    plt.figure(figsize=(8, 12))
    plt.subplot(2, 1, 1)
    plt.title('Spectrogram Predicted Voice')
    librosa.display.specshow(stftout_mag_db, x_axis='time', y_axis='linear', sr=sample_rate,
                             hop_length=hop_length_fft)
    plt.colorbar()
    plt.subplot(2, 1, 2)
    plt.title('Spectrogram Clean Voice')
    librosa.display.specshow(stftvoice_mag_db, x_axis='time', y_axis='linear', sr=sample_rate,
                             hop_length=hop_length_fft)
    plt.colorbar()
    plt.tight_layout()
...
def make_2plots_timeseries_voice_after(clipvoicenoise, clipafter):
    fig, ax = plt.subplots(figsize=(12, 6))
    plt.title('Audio')
    plt.ylabel('Amplitude')
    plt.xlabel('Time(s)')
    ax.plot(clipvoicenoise)
    ax.plot(clipafter, alpha=0.5)
    plt.show()
